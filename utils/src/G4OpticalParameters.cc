...
void G4OpticalParameters::SetResFromFunction(G4bool val)
{
  if(IsLocked())
  {
    return;
  }
  resolutionFromFunction = val;
}

G4bool G4OpticalParameters::GetResFromFunction() const
{
  return resolutionFromFunction;
}
...
void G4OpticalParameters::StreamInfo(std::ostream& os) const
{
  ...
  os << " Scintillation by particle type:        " << scintByParticleType
     << "\n";
  os << " Scintillation resolution from function: " << resolutionFromFunction << "\n";
